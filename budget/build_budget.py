...
DEPT_CODES = OrderedDict([
    ...
    ('松江・境港',      ['0032', '0033', '0034', '0035', '0036', '0037', '0038', '0039',
                        '0903', '0904', '0909', '0910', '0911', '0914', '0915', '0916',
                        '0918', '0919', '0930']),   # 0930 自治体(境港市) は新規コード
...
# ---------- 見出し ----------
PREV_MONTHS = ['2024/10', '2024/11', '2024/12', '2025/01', '2025/02', '2025/03']  # 前年度（第51期上期）
CUR_MONTHS  = ['2025/10', '2025/11', '2025/12', '2026/01', '2026/02', '2026/03']  # 今年度（第52期上期）
HEADERS = (['得意先コード', '得意先略称', '担当者コード', '担当者名', '新担当者コード', '新担当者']
           + [m + '売上' for m in PREV_MONTHS] + ['前年度売上合計']
           + [m + '粗利益' for m in PREV_MONTHS] + ['前年度粗利益合計']
           + [m + '売上' for m in CUR_MONTHS] + ['今年度売上合計']
           + [m + '粗利益' for m in CUR_MONTHS] + ['今年度粗利益合計'])
...
ws_all['AI1'].comment = Comment('担当者コードから判定した主部門。0017 鮮魚は参考ファイル同様 水産部X・石見 の両シートに掲載していますが、主部門は水産部X です。', 'Claude')
    ...
    for code in codes:
        ...
        for rec in recs:
            if dept == '石見' and code == '0017':
                ...
                sl = DEPT_LAST['水産部X']
                f = (f'=IFERROR(IF(INDEX(水産部X!$E$2:$E${sl},MATCH($A{r},水産部X!$A$2:$A${sl},0))="","",'
                     f'INDEX(水産部X!$E$2:$E${sl},MATCH($A{r},水産部X!$A$2:$A${sl},0))),"")')
